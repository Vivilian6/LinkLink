...
public class StartAnimationActivity extends Activity
{
    ...
    private ImageView ivAction; //用于播放action动画的ImageView
    private ImageButton ibLogin; //用于播放login动画的ImageButton
    private AnimationDrawable ad;   //ImageView的内部动态壁纸
    private Animation am;   //alpha动画
    private Animation amAction; //scale动画
    private Animation amLogin; //scale动画
    ...
    private void init()
    {
        ...
        ivAction = (ImageView) findViewById(R.id.action);
        ibLogin = (ImageButton) findViewById(R.id.login);

        ivAction.setBackgroundResource(R.drawable.action);
        ibLogin.setBackgroundResource(R.drawable.login);

        ad = (AnimationDrawable) iv.getBackground();

        am = AnimationUtils.loadAnimation(this, R.anim.alphaanimation);
        amAction = AnimationUtils.loadAnimation(this,R.anim.scaleanimation);
        amLogin = AnimationUtils.loadAnimation(this,R.anim.scale2animation);
        //绑定动画
        iv.setAnimation(am);
        ...
        am.setAnimationListener(new Animation.AnimationListener() {
            ...
            public void onAnimationEnd(Animation animation) {
                ivAction.setAnimation(amAction);
        ...
        //action动画结束后显示login动画
        amAction.setAnimationListener(new Animation.AnimationListener() {
            @Override
            public void onAnimationStart(Animation animation) {
            }
            @Override
            public void onAnimationEnd(Animation animation) {
                ibLogin.setAnimation(amLogin);
